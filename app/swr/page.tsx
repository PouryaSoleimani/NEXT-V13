      ...
      <div className='grid grid-cols-4 p-3'>
        {users?.slice(0, 5)?.map((user: SingleUserType) => (
          <div key={user.id} className='p-2 font-mono font-semibold text-lg border-b-4 border-r-4 border-t-4 border-zinc-600 first:border-l-4 last:border-l-4 last:border-t-0 '>
            <h3 className='flex items-center gap-3 uppercase'><User className='size-5' />{user.name.firstname} {user.name.lastname}</h3>
            <p className='text-blue-700 flex items-center gap-3'><Mail className='size-5' />{user.email}</p>
            <p className='text-green-800 flex items-center gap-3'><Phone className='size-5' />{user.phone}</p>
            <p className='text-red-700/50 hover:text-red-700 flex items-center gap-3'><Lock className='size-5' />{user.password}</p>
          </div>
        ))}
      </div>
    </div>
