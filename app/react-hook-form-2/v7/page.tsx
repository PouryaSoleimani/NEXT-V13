...
const ReactHookFormV7 = () => {
  ...
  function submitApi() {
    axios
      ...
      .catch((err) => {
        ...
        methods.setError("root", { type: "deps", message: "Network Problem" });
      });
  }
  ...
  function submitHandler(data: FormTypesv7) {
    ...
    toast.success(
      `FORM SUBMITTED  =>  Title :${data.title} Price:$${data.price}`,
      { position: "top-center", style: { width: "500px" } }
    );
    methods.reset();
  }

  const titleValue = useWatch({ name: "title", control: methods.control });

  const priceValue = useWatch({ control: methods.control, name: "price" });
