...
const ReactHookFormV5 = () => {
   ...
   const {
      ...
      defaultValues: { skills: [{ title: "", level: null, experiences: [{ company: "", years: null }] }] },
   });
       ...
       <div className="section bg-black min-h-screen p-8">
           <form onSubmit={handleSubmit(SubmitHandler)} className="max-w-4xl mx-auto">
               ...
               {skillFields.map((skill, index) => (
                   <SkillItem key={skill.id} control={control} register={register} errors={errors} index={index} removeSkill={removeSkill} />
               ))}

               <div id="BUTTONS__CONTAINER" className={cn("flex gap-3 w-[50%] mx-auto  py-5", skillFields.length === 0 && "w-full")}>
