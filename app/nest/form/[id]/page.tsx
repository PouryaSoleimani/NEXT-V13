...
const NestForm = () => {
  ...
  async function submitHandler(values: any) {
    if (isEdit) {
      ...
      await axios
        .put(`http://localhost:5000/jojos/${data?.id}`, values)
        .then((res) => console.info("RES UPDATE ==>", res.data))
      ...
      console.info(values);
      axios
        .post("http://localhost:5000/jojos/create", values)
        .then((res) => console.info("RES ==>", res.data))
        .catch((err) => err.message);
      form.reset();
      router.push("/nest/");
    }
